...
public class Block {
    ...
    public static Block createBlock() {
        ...
        int shapeIndex = random.nextInt(com.example.tetrisnoactivity.models.Shape.values().length);
        BlockColor blockColor = BlockColor.values()[random.nextInt(BlockColor
                .values().length)];

        Block block = new Block(shapeIndex, blockColor);
        block.position.x = block.position.x - com.example.tetrisnoactivity.models.Shape.values()[shapeIndex].getStartPosition();
    ...
    public final byte[][] getShape(int frameNumber) {
        return com.example.tetrisnoactivity.models.Shape.values()[shapeIndex].getFrame(frameNumber).as2dByteArray();
    }
